...
class mixture_linear():
    ...
    def __init__(self, session, lr, l2, batch_size, order_auto, order_x, order_steps, bool_log, bool_bilinear, \
        ...
        self.auto = tf.placeholder(tf.float32, [None, order_auto])
        self.y = tf.placeholder(tf.float32, [None, ])
        self.keep_prob = tf.placeholder(tf.float32)
        
        if bool_bilinear == True:
            self.x = tf.placeholder(tf.float32, [None, order_steps, order_x])
        else:
            self.x = tf.placeholder(tf.float32, [None, order_x])
        
        
        # --- prediction of individual models

        # models on individual feature groups
        mean_v, regu_v_mean = linear_predict(self.auto, order_auto, 'mean_v', True)
        
        if bool_bilinear == True:
            mean_x, regu_x_mean = bilinear(self.x, [order_steps, order_x], 'mean_distr', True)
        else:
            mean_x, regu_x_mean = linear_predict(self.x, order_x, 'mean_distr', True)
        ...
        if activation_type == 'relu':
            ...
            mean_x = tf.nn.leaky_relu(mean_x)
        ...
        varv, regu_v_var = linear(self.auto, order_auto, 'sig_v', True)
        
        # only linear 
        vardistr, regu_x_var = linear( tf.reshape(self.x, [-1, order_steps*order_x]), order_steps*order_x, 'sig_distr', True)
        ...
        if bool_bilinear == True:
            ...
            self.logit, regu_gate = bilinear_with_external( self.distr, [order_steps, order_x], \
                                                            self.auto, order_auto, 'gate_distr', True )
            
            #bilinear(self.distr, [order_steps, order_x], 'gate_distr', True)
            
        else:
            # ?
            self.logit, regu_gate = linear(self.distr, order_x, 'gate_distr', True)
        
        self.gates = tf.stack( [tf.sigmoid(self.logit), 1.0 - tf.sigmoid(self.logit)] ,1 )
        '''
        
        # gate based on softmax        
        logit_v, regu_v_gate = linear(self.auto, order_auto, 'gate_v', True)
        
        if bool_bilinear == True:
            # ?
            logit_x, regu_x_gate = bilinear(self.x, [order_steps, order_x], 'gate_x', True)
            
        else:
            logit_x, regu_x_gate = linear(self.x, order_x, 'gate_x', True)
...
#   initialize loss and optimization operations for training
    ...
    def inference(self, auto_test, x_test, y_test, keep_prob):
        ...
        return self.sess.run([self.mse, self.regu], feed_dict = {self.auto:auto_test, \
...
class variational_mixture_linear():
    ...
    def __init__(self, session, lr, batch_size, order_auto, order_x, order_steps, bool_log, bool_bilinear, \
                 distr_type, eval_sample_num, infer_type, num_iter):
        ...
        # models on individual feature groups
        mean_v = bayesian_linear(self.auto, order_auto, 'mean_v', True, self.dict_var, None, None, infer_type)
        
        if bool_bilinear == True:
            mean_x = bayesian_bilinear(self.x, [order_steps, order_x], 'mean_distr', True, self.dict_var, None,\
                                           None, None, infer_type)
        else:
            mean_x = bayesian_linear(self.x, order_x, 'mean_distr', True, self.dict_var, None, None, infer_type)

        # concatenate individual means 
        # [N, 2]
        mean_stack = tf.concat( [mean_v, mean_x], 1 )
        
        
        # --- variance of individual models
        
        # variance depedent on features 
        varv = bayesian_linear(self.auto, order_auto, 'sig_v', True, self.dict_var, None, None, infer_type)
        
        flatten_d = tf.reshape(self.x, [-1, order_steps*order_x])
        vardistr = bayesian_linear(flatten_d, order_steps*order_x, 'sig_distr', True, self.dict_var, None, None,\
                                   infer_type)
        ...
        # concatenate individual variance 
        # [N, 2]
        var_stack = tf.concat( [var_v, var_x], 1 )
        
        
        # --- gate weights of individual models
        
        # gate based on softmax        
        logit_v = bayesian_linear(self.auto, order_auto, 'gate_v', True, self.dict_var, None, None, infer_type)
        
        if bool_bilinear == True:
            logit_x = bayesian_bilinear(self.x, [order_steps, order_x], 'gate_distr', True, self.dict_var, None, None,\
                                        None, infer_type)
        else:
            logit_x = bayesian_linear(self.x, order_x, 'gate_distr', True, self.dict_var, None, None, infer_type)
        
        # [N, 2]
        self.logits = tf.concat([logit_v, logit_x], 1)
    ...
    def train_batch(self, v_train, distr_train, y_train, keep_prob):
        ...
        # -- test
        info_dict = self.inference.update({self.auto : v_train, self.x : distr_train, self.y : y_train, \
    ...
    def evaluate_ini(self):
        ...
        for i in range(self.eval_sample_num):
            ...
            mean_v = bayesian_linear(self.auto, 0, '', True, {}, tf.cast(v_w, tf.float32), tf.cast(v_b, tf.float32), '')
            mean_d = bayesian_bilinear(self.x, 0, '', True, {}, tf.cast(d_w_l, tf.float32), tf.cast(d_w_r, tf.float32),\
                                     tf.cast(v_b, tf.float32))
            
            logit_v = bayesian_linear(self.auto, 0, '', True, {}, tf.cast(gate_v_w, tf.float32), \
                                      tf.cast(gate_v_b, tf.float32), '')
            logit_x = bayesian_bilinear(self.x, 0, '', True, {}, tf.cast(gate_d_w_l, tf.float32), 
                                        tf.cast(gate_d_w_r, tf.float32),\
                                        tf.cast(gate_d_b, tf.float32))
            
            # [N, 2]
            logits = tf.concat([logit_v, logit_x], 1)
    ...
    def predict(self, v_test, distr_test, keep_prob):
        ...
        return self.sess.run([self.y_hat, self.pre_v, self.pre_x], feed_dict = {self.auto:v_test, \
                                                       self.x:distr_test,  self.keep_prob:keep_prob })
    
    #   mixture gates givn testing data
    def predict_gates(self, v_test, distr_test, keep_prob):
        return self.sess.run( self.gates , feed_dict = {self.auto:v_test, \
                                                        self.x:distr_test,  self.keep_prob:keep_prob })
